...
sheet = client.open("maskupfine").sheet1
...
def none():
    ...
    for row in range(2,nrows+1):
        sheet.update_cell(row,datecell.col,'Rs. 0') 
...
def write_to_sheet(name):
	...
	if(sheet.cell(namecell.row,datecell.col).value =='Rs. 0' ):
		if(time<max_intime):
			sheet.update_cell(namecell.row,datecell.col,'Rs. 200') 
			print('recorded')
